...
do
    ...
    mkdir -p "$targetFolder"/"$type"/"$sid"
    if [ "$type" = "C" ]; then
        cp "$fileName" "$targetFolder"/"$type"/"$sid"/main.c
    elif [ "$type" = "Python" ]; then
        cp "$fileName" "$targetFolder"/"$type"/"$sid"/main.py
    elif [ "$type" = "Java" ]; then
        cp "$fileName" "$targetFolder"/"$type"/"$sid"/Main.java
    fi

    if [ "$noExecute" = false ]; then
        print_info "Executing files of $sid"
        if [ "$type" = "C" ]; then
            gcc "$targetFolder"/"$type"/"$sid"/main.c -o "$targetFolder"/"$type"/"$sid"/main.out
        elif [ "$type" = "Java" ]; then
            javac "$targetFolder"/"$type"/"$sid"/Main.java
        fi
        ...
        do
            tmp=${testFile%.txt}
            testNum=${tmp:10}
            outputFile="$targetFolder"/"$type"/"$sid"/out"$testNum".txt
            if [ "$type" = "C" ]; then
                "$targetFolder"/"$type"/"$sid"/main.out < "$testFile" > "$outputFile"
            elif [ "$type" = "Python" ]; then
                python3 "$targetFolder"/"$type"/"$sid"/main.py < "$testFile" > "$outputFile"
            elif [ "$type" = "Java" ]; then
                java -cp "$targetFolder"/"$type"/"$sid" Main < "$testFile" > "$outputFile"
            fi
